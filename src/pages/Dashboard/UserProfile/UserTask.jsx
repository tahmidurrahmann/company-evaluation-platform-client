...
const UserTask = () => {
  ...
  const handleMoveTask = async (e) => {
    ...
    const res = await axiosSecure.put(
      `/moveTask?task=${status}&id=${selectedValue}`
    );
  ...
  useEffect(() => {
    if (specificEmployee?.length > 0) {
      const todoTask = specificEmployee?.filter(
        (employee) =>
          employee?.status === "todo" && employee.email === user.email
      );
      setTodo(todoTask);
      const doingTask = specificEmployee?.filter(
        (employee) =>
          employee?.status === "doing" && employee.email === user.email
      );
      setDoing(doingTask);
      const completedTask = specificEmployee?.filter(
        (employee) =>
          employee?.status === "completed" && employee.email === user.email
      );

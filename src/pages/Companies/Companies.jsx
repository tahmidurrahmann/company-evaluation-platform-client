...
const Companies = () => {
  ...
  useEffect(() => {
    if (hrInfo?.length > 0) {
      const allCheckedCompanyInfo = hrInfo?.filter(
  ...
  return (
    <div className="">
      ...
      <div className="px-6 mx-auto max-w-screen-2xl xl:px-0">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 md:gap-8 lg:gap-12">
          <div className="grid grid-cols-1 col-span-2 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {companies?.map((company) => (
              <div
                className="p-6 my-2 border rounded-lg lg:my-6"
                key={company?._id}
              >
                ...
                <h1>
                  Year Founded: {company?.yearFounded?.split("-")[0]}, Month:{" "}
                  {company?.monthName}
                </h1>
              </div>
            ))}
